...
export default function FollowersPage() {
  ...
  useEffect(() => { apiGet<any>(`/users/${id}/followers`).then(r => setUsers(r.data)).catch(() => {}); }, [id]);

  return (
    <div className="mx-auto max-w-lg px-4 py-8">
      <Link href={`/user/${id}`} className="mb-4 block text-sm text-blue-600 hover:underline">← 返回主页</Link>
      <h1 className="mb-4 text-xl font-bold text-gray-900">粉丝</h1>
      {users.length === 0 ? <p className="text-gray-400 text-center py-8">暂无粉丝</p> : users.map(u => (
